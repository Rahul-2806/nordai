...
const SUGGESTIONS = [
  ...
  'What drives Spanish prices low?',
...
export default function CopilotFloat() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Good morning. I have live data on all European electricity markets. Ask me anything — prices, signals, arbitrage, or market drivers.' },
  ])
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const res = await chatWithCopilot(msg, history)
  ...
  return (
    <>
      ...
      {open && (
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <div style={{ fontFamily: "'Playfair Display', serif", fontSize: 15, color: 'var(--cream)' }}>NordAI Co-Pilot</div>
            ...
            <button onClick={() => setOpen(false)} style={{
              ...
              color: 'var(--ink4)', cursor: 'pointer', fontSize: 18, lineHeight: 1,
          ...
          <div style={{
            ...
            {messages.map((m, i) => (
              <div key={i} style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                {m.role === 'assistant' && (
                  <div style={{ fontSize: 9, letterSpacing: '1px', color: 'var(--ink4)', textTransform: 'uppercase', paddingLeft: 2 }}>NordAI</div>
                )}
                <div style={{
                  maxWidth: '88%',
                  ...
                  padding: '9px 13px',
                  fontSize: 12, color: m.role === 'user' ? 'var(--ink)' : 'var(--cream2)',
          ...
          {/* Input */}
          <div style={{ padding: '10px 14px 14px', display: 'flex', gap: 8, borderTop: '1px solid rgba(255,255,255,0.06)' }}>
            <input
              value={input}
              ...
              style={{
                ...
                color: 'var(--cream2)', fontSize: 12, fontFamily: 'Syne', outline: 'none',
              }}
            />
            <button onClick={() => send()} disabled={loading} style={{
              ...
              fontFamily: 'Syne', opacity: loading ? 0.6 : 1,
              flexShrink: 0,
            }}>→</button>
          </div>
        </div>
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        ...
        fontSize: 20, color: open ? 'var(--cream)' : 'var(--ink)',
        fontWeight: 600,
      }}
